...
<head>
	...
	<?php
		...
		// Inicializar $plantilla con valores por defecto
		$plantilla = array(
			"pixelFacebook" => "",
			"apiFacebook" => "",
			"googleAnalytics" => "",
			"icono" => "",
			"logo" => "",
			"barraSuperior" => COMPANY_NAME,
			"textoSuperior" => COMPANY_SLOGAN,
			"colorFondo" => "",
			"colorTexto" => "",
			"redesSociales" => json_encode([
				'facebook' => APP_SOCIAL_FACEBOOK,
				'twitter' => APP_SOCIAL_TWITTER,
				'instagram' => APP_SOCIAL_INSTAGRAM,
				'youtube' => APP_SOCIAL_YOUTUBE
			])
		);

		// Intentar obtener los datos de la plantilla
		$controladorPlantilla = new ControladorPlantilla();
		$datosPlantilla = $controladorPlantilla->ctrEstiloPlantilla();
		if($datosPlantilla) {
			$plantilla = array_merge($plantilla, $datosPlantilla);
		}

		echo '<link rel="icon" href="'.BACKEND_URL.$plantilla["icono"].'">';
		...
		// Valores por defecto para las cabeceras
		$cabecerasDefault = array(
			"ruta" => "inicio",
			"titulo" => COMPANY_NAME,
			"descripcion" => COMPANY_DESCRIPTION,
			"palabrasClaves" => "tienda, virtual, productos",
			"portada" => ""
		);

		// Si no hay cabeceras o no tienen la ruta, usar valores por defecto
		if(!$cabeceras || !isset($cabeceras["ruta"])) {
			$ruta = "inicio";
			$cabeceras = ControladorPlantilla::ctrTraerCabeceras($ruta);
			if(!$cabeceras) {
				$cabeceras = $cabecerasDefault;
			}
		}

		// Asegurarnos de que todos los campos necesarios existan
		$cabeceras = array_merge($cabecerasDefault, $cabeceras);
	...
	======================================-->
	<script>
		var rutaOculta = '<?php echo isset(
			$url) ? $url : "/ROPAAFRICANA/frontend/"; ?>';
		console.log('rutaOculta definida como:', rutaOculta);
	</script>

	<!--=====================================
	HOJAS DE JAVASCRIPT PERSONALIZADAS
	======================================-->
...
if(isset($_GET["ruta"])){
	...
	if($rutaCategorias && isset($rutaCategorias["ruta"]) && $rutas[0] == $rutaCategorias["ruta"]){
	...
	if($rutaProductos && isset($rutaProductos["ruta"]) && $rutas[0] == $rutaProductos["ruta"]){
	...
	}else if($rutas[0] == "buscador" || $rutas[0] == "verificar" || $rutas[0] == "salir" || $rutas[0] == "perfil" || $rutas[0] == "carrito-de-compras" || $rutas[0] == "error" || $rutas[0] == "finalizar-compra" || $rutas[0] == "curso" || $rutas[0] == "ofertas" || $rutas[0] == "politica-privacidad"){
