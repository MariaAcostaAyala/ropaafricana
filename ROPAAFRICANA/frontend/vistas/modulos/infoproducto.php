	...
	<div class="container">
		...
		<div class="row">
			...
			<ul class="breadcrumb fondoBreadcrumb text-uppercase">
				...
				<li class="active pagActiva"><?php echo isset($rutas[0]) ? $rutas[0] : 'Producto'; ?></li>
	...
	<div class="container">
		...
		<div class="row">
			...
			<?php
				...
				$item =  "ruta";
				$valor = isset($rutas[0]) ? $rutas[0] : '';
				$infoproducto = ControladorProductos::ctrMostrarInfoProducto($item, $valor);

				echo "<!-- Debug: rutas[0] = ".$valor." -->";
				echo "<!-- Debug: infoproducto = ".print_r($infoproducto, true)." -->";

				// Verificar que $infoproducto existe y tiene la clave multimedia
				if($infoproducto && isset($infoproducto["multimedia"])) {
					$multimedia = json_decode($infoproducto["multimedia"], true);
					echo "<!-- Debug: multimedia original = ".$infoproducto["multimedia"]." -->";
					echo "<!-- Debug: multimedia decodificado = ".print_r($multimedia, true)." -->";
				} else {
					$multimedia = array();
					echo "<!-- Debug: multimedia está vacío -->";
				}
				...
				if(isset($infoproducto["tipo"]) && $infoproducto["tipo"] == "fisico"){
					echo "<!-- Debug: tipo es físico -->";
					echo '<div class="col-md-5 col-sm-6 col-xs-12 visorImg">
						
							<figure class="visor">';

							if($multimedia && is_array($multimedia)){
								echo "<!-- Debug: multimedia es un array -->";
								for($i = 0; $i < count($multimedia); $i ++){
									echo "<!-- Debug: Iteración ".$i." -->";
									if(is_string($multimedia[$i])) {
										$rutaImagen = BACKEND_URL.$multimedia[$i];
										$rutaArchivo = dirname(dirname(dirname(dirname(__FILE__)))).'/backend/'.$multimedia[$i];
										echo "<!-- Debug: rutaImagen = ".$rutaImagen." -->";
										echo "<!-- Debug: rutaArchivo = ".$rutaArchivo." -->";
										echo "<!-- Debug: ¿Existe el archivo?: ".(file_exists($rutaArchivo) ? "Sí" : "No")." -->";
										if(file_exists($rutaArchivo)) {
											echo '<img id="lupa'.($i+1).'" class="img-thumbnail" src="'.$rutaImagen.'">';
										} else {
											echo '<img id="lupa'.($i+1).'" class="img-thumbnail" src="'.BACKEND_URL.'vistas/img/productos/default.jpg">';
										}
									} else {
										echo "<!-- Debug: multimedia[".$i."] no es una cadena: ".print_r($multimedia[$i], true)." -->";
									}
								}								
								...
								for($i = 0; $i < count($multimedia); $i ++){
									...
									if(isset($multimedia[$i]["foto"])) {
										$rutaImagen = BACKEND_URL.$multimedia[$i]["foto"];
										$rutaArchivo = dirname(dirname(dirname(dirname(__FILE__)))).'/backend/'.$multimedia[$i]["foto"];
										if(file_exists($rutaArchivo)) {
											echo '<li>
									     	<img value="'.($i+1).'" class="img-thumbnail" src="'.$rutaImagen.'" alt="'.(isset($infoproducto["titulo"]) ? $infoproducto["titulo"] : '').'">
									    </li>';
										} else {
											echo '<li>
									     	<img value="'.($i+1).'" class="img-thumbnail" src="'.BACKEND_URL.'vistas/img/productos/default.jpg" alt="'.(isset($infoproducto["titulo"]) ? $infoproducto["titulo"] : '').'">
									    </li>';
										}
									}
					...
					echo '<div class="col-sm-6 col-xs-12">
						...
						<iframe class="videoPresentacion" src="https://www.youtube.com/embed/'.(isset($infoproducto["multimedia"]) ? $infoproducto["multimedia"] : '').'?rel=0&autoplay=0" width="100%" frameborder="0" allowfullscreen></iframe>
				...
				<?php
					...
					echo '<div class="comprarAhora" style="display:none">
						...
						if($infoproducto["oferta"] == 0){
							...
							<p class="subTotal'.$infoproducto["id"].' subtotales">
								...
								<strong>EUR €<span>'.$infoproducto["precio"].'</span></strong>
							...
							<p class="subTotal'.$infoproducto["id"].' subtotales">
								...
								<strong>EUR €<span>'.$infoproducto["precioOferta"].'</span></strong>
					...
					if($infoproducto["precio"] == 0){
						...
						if($infoproducto["oferta"] == 0){
							...
							echo '<h2 class="text-muted">EUR €'.$infoproducto["precio"].'</h2>';
							...
							echo '<h2 class="text-muted">
								...
								<span>
									...
									<strong class="oferta">EUR €'.$infoproducto["precio"].'</strong>
				...
				<?php
					...
					if($infoproducto["detalles"] != null){
						...
						if($infoproducto["tipo"] == "fisico"){
							...
							if(isset($detalles["Talla"]) && $detalles["Talla"] != null){
							...
							if(isset($detalles["Color"]) && $detalles["Color"] != null){
							...
							if(isset($detalles["Marca"]) && $detalles["Marca"] != null){
				...
				<?php
					...
					if($infoproducto["precio"]==0){
						...
						if($infoproducto["tipo"]=="virtual"){
							...
							if(!isset($_SESSION['validarSesion']) || $_SESSION['validarSesion'] !== true){
								echo '<button type="button" class="btn btn-default btn-block btn-lg backColor" 
									onclick="window.location.href=\''.$url.'ingreso\'">
									<i class="fa fa-user" aria-hidden="true"></i> INICIAR SESIÓN PARA COMPRAR
								</button>';
							} else {
								if($infoproducto["oferta"] != 0){
									echo '<button type="button" class="btn btn-default btn-block btn-lg backColor agregarCarrito" 
							...
							if($value["oferta"] != 0){
								...
								echo '<h2>
										...
										<small>
											...
											<strong class="oferta">EUR €'.$value["precio"].'</strong>
								...
								echo '<h2><small>EUR €'.$value["precio"].'</small></h2>';
