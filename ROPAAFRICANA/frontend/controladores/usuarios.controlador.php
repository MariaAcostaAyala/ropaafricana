...
class ControladorUsuarios{
	...
	static public function ctrRegistroUsuario(){
		...
		try {
			error_log("Iniciando registro de usuario...");
			error_log("Datos POST recibidos: " . print_r($_POST, true));

			if(isset($_POST["regUsuario"]) && isset($_POST["regEmail"]) && isset($_POST["regPassword"])){
				...
				$datos = array(
					...
					"modo" => "directo",
					"verificacion" => 0,
					"emailEncriptado" => md5($_POST["regEmail"])
				);

				error_log("Datos a insertar: " . print_r($datos, true));

				$respuesta = ModeloUsuarios::mdlRegistroUsuario("usuarios", $datos);
				error_log("Respuesta del modelo: " . print_r($respuesta, true));
				...
				if($respuesta[0] == "ok"){
					...
					echo '<script>
						swal({
							title: "¡OK!",
							text: "¡Registro exitoso! Serás redirigido a tu perfil",
							type:"success",
				...
				}else if($respuesta == "email_duplicado"){
					echo '<script>
						swal({
							title: "¡Error!",
							text: "El email ya está registrado. Por favor, use otro email o inicie sesión",
							type: "error",
							confirmButtonText: "Cerrar",
							closeOnConfirm: false
						},
					...
					echo '<script>
						swal({
							title: "¡Error!",
							text: "Ha ocurrido un error al registrar el usuario. Por favor, intente nuevamente",
							type: "error",
							confirmButtonText: "Cerrar"
						});
					</script>';
			...
			error_log("Error en ctrRegistroUsuario: " . $e->getMessage());
			echo '<script>
				swal({
					title: "¡Error!",
			...
			if(preg_match('/^[^0-9][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[@][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[.][a-zA-Z]{2,4}$/', $_POST["passEmail"])){
				...
				if($respuesta1){
					...
					if($respuesta2  == "ok"){
						...
						$mail->setFrom(Configuracion::$email['email_remitente'], Configuracion::$email['nombre_remitente']);

						$mail->addReplyTo(Configuracion::$email['email_respuesta'], Configuracion::$email['nombre_respuesta']);
